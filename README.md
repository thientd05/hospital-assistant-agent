...
[![Xem demo trên YouTube](https://img.shields.io/badge/▶_Xem_Demo-YouTube-red?style=for-the-badge&logo=youtube)](https://youtu.be/Qr-uZIxU1r8)

> 📺 **Link demo:** https://youtu.be/Qr-uZIxU1r8
...
FamilyHealthAI là **co-pilot** cho một chuỗi phòng khám gia đình, phục vụ trực tiếp hai vai trò ở mặt tiền khám chữa bệnh:

- **Bác sĩ** — thay vì lọ mọ qua hàng loạt form, bác sĩ ra lệnh bằng tiếng Việt tự nhiên: *"xếp bệnh nhân này vào khoa tim mạch, ghi sinh hiệu mạch 92 huyết áp 150/95"*, *"kê Amlodipin + Atorvastatin"*, *"vẽ lịch sử khám gần đây"*. Agent tự mở hồ sơ lâm sàng, điền sinh hiệu, nhập kết quả xét nghiệm, kê đơn (**tự kiểm tra tương tác thuốc** khi chọn ≥2 thuốc), duyệt/nhận lịch hẹn, dựng dashboard lịch sử khám — tất cả qua đúng giao diện mà bác sĩ vẫn thao tác bằng tay.
- **Bệnh nhân** — tự đăng ký bằng số điện thoại rồi được trợ lý dẫn dắt: hỏi đáp sức khỏe, khai báo thông tin cá nhân, đặt lịch khám, xem hồ sơ và kết quả xét nghiệm của chính mình. Agent của bệnh nhân dùng **chung một cỗ máy** với agent bác sĩ nhưng bị giới hạn quyền chặt chẽ — chỉ chạm được đúng phần dữ liệu của họ.

Điểm mấu chốt: cả hai không "điền form" — họ **nói**, agent **làm**, và làm bằng cách bấm đúng những nút mà con người bấm.

### Vì sao không làm theo cách thông thường
...
Dự án này tiếp cận khác: **agent điều khiển chính giao diện của ứng dụng**, giống một coding-agent điều khiển repo/trình duyệt. Agent không có tool "kê đơn"; nó có tool "nhìn màn hình" và "click/gõ", rồi *học* cách kê đơn từ một tài liệu kỹ năng. Mọi thao tác đi qua đúng các handler, validation và REST call mà người dùng thật cũng đi qua.

---

## Lấy cảm hứng từ Claude Code

Một coding-agent như Claude Code không có hàm `fixBug()` — nó có `Read`/`Edit`/`Bash` và *học* cách sửa bug từ ngữ cảnh. Dự án bê nguyên triết lý đó sang phần mềm nghiệp vụ: **agent vận hành UI như con người, năng lực đến từ dữ liệu chứ không phải code**. Toàn bộ các primitive hiện đại được tái hiện:

| Primitive (Claude Code) | Hiện thực ở đây |
|---|---|
| **Agent loop** — vòng lặp tool-use, streaming | `loop.ts`: stream SSE từng frame, re-check allowlist mỗi block, prompt-cache phân tầng (xem dưới) |
| **Tools** — `Read`/`Edit`/`Bash`… generic | Bộ tool mô phỏng con người, KHÔNG theo nghiệp vụ: `read_panel` (nhìn), `act` (click/gõ/select/check), `read_skills`, `update_workspace_file`, `read_exam_history`, `read_service_prices` |
| **Skills** — Markdown + progressive disclosure | Mỗi nghiệp vụ = 1 file Markdown (`name` + `description`) trong Mongo. Loop chỉ nạp *index* từ description (rẻ); body đọc **lazy** đúng lúc qua `read_skills`. Thêm/sửa nghiệp vụ = sửa skill, áp dụng ngay lượt sau — **không deploy lại code** |
| **Hooks** — PreToolUse / PostToolUse | Hook framework xen quanh tool, khai báo `{tool, timing: before\|after, match}`. VD `current-datetime` bơm ngày giờ VN *trước* `read_skills('book-appointment')` để agent tự quy đổi "tuần sau" |
| **CLAUDE.md / Memory** — bộ nhớ bền tự sửa | Workspace `{user, soul, memory}` nối vào system prompt mỗi lượt; agent **tự ghi** bài học khi mắc lỗi qua `update_workspace_file({file:"memory"})`, sửa hành vi mà không cần can thiệp code |
| **Permissions / allowlist** | `access.ts` + `config.json`: allowlist tool/skill theo role, tách hẳn khỏi route-guard `requireRole` ở REST → hai tầng phân quyền độc lập |
| **Computer use / accessibility tree** | Agent "nhìn" panel qua *snapshot* `{ref, role, label, value, disabled}` thay vì HTML thô; nhắm action bằng `ref` ổn định → không bịa selector |
| **Sub-agent / context isolation** | Agent là microservice **stateless** tách rời REST: verify JWT cục bộ, không chạm DB, mọi persistence forward qua REST |
| **Prompt caching** phân tầng | System prompt tách quanh cache-breakpoint: phần `stable` (boot + index skill + hồ sơ) mang breakpoint, `memory` động nối sau → đổi memory không bust cache phần ổn định |
| **Artifacts** | Agent nhúng **SVG sketch** stream tăng dần từng token (sanitize DOMPurify) ngay trong khung chat để trực quan hóa hồ sơ/lịch hẹn |
