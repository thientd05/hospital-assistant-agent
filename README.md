...
- 🧩 **Skill động lưu ngoài codebase** — kỹ năng nghiệp vụ của agent được viết bằng Markdown, lưu trong MongoDB, soạn/sửa qua UI quản trị (hoặc REST). Thêm năng lực mới cho agent **không cần deploy lại code**.
...
## 🎭 Một agent, hai ngữ cảnh — cùng bộ tool

Điểm thú vị: **cùng một agent loop** phục vụ hai vai trò với quyền hạn khác nhau, chỉ khác nhau ở allowlist tool/skill và phạm vi panel:
...
> Bệnh nhân có **đủ bộ tool panel** như bác sĩ, nhưng allowlist + snapshot + schema strict khiến nó chỉ thao tác được đúng 5 trường hồ sơ của chính mình. Cùng cỗ máy, khác phạm vi — một minh chứng cho thiết kế phân quyền agent chặt chẽ.
...
Kỹ năng agent là Markdown có frontmatter (`name` / `description`), lưu ở collection `skills`. Loop chỉ build *index* từ description (rẻ); body được nạp lazy khi agent gọi `read_skills`. Triết lý: **skill ngắn, chia nhỏ theo từng nhiệm vụ** — đọc một phát là gen được trọn chuỗi `read_panel → act`, tối thiểu số bước. Skill soạn/sửa runtime qua UI quản trị (hoặc REST), thêm năng lực mới không cần kỹ sư deploy.
</details>
...
- ✅ **Prompt-ops runtime** — skill soạn/sửa qua UI, thêm năng lực không cần deploy
