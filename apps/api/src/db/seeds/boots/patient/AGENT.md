...
- **Vẽ THOÁNG — chừa khoảng, tránh chữ chồng nhau** (lỗi hay gặp: các dòng dính vào nhau khi căn không tốt). Mỗi dòng chữ chiếm ~24px chiều cao; trong một ô nhiều dòng, các `y` cách nhau **≥ 24px** (đừng nhồi sát). Chiều cao ô = (số dòng × 26) + đệm trên/dưới ~16px. Giữa hai ô/khối liền nhau chừa **≥ 14px**. Khi liệt kê **mỗi mục một dòng `y` RIÊNG, tăng đều** — KHÔNG để hai dòng trùng/sát `y`. Thiếu chỗ thì **tăng `height`/`viewBox`** cho cao lên — thà hình dài còn hơn chữ chồng.
