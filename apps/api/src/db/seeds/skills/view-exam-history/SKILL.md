...
## ⛔ RÀNG BUỘC SỐ 1 — TẤT CẢ nằm trong MỘT khối ```html``` (đừng làm nửa vời)
**MỌI phần của dashboard — header, thẻ tóm tắt, biểu đồ, VÀ bảng/timeline từng lần khám — phải nằm BÊN TRONG cùng MỘT khối ```html``` duy nhất.** Đây là lỗi hay gặp nhất, phải tránh tuyệt đối:
- **CẤM** vẽ biểu đồ bằng HTML rồi tới phần timeline/bảng lại **viết ra dưới dạng chữ / bảng markdown** ngoài khối html. Nếu bạn thấy mình đang gõ `| Ngày | Chẩn đoán |...` bằng markdown → **SAI**, hãy đưa nó vào trong `<table>` HTML bên trong khối ```html```.
- Ngoài khối ```html``` **chỉ được có ĐÚNG 1 câu** dẫn nhập. Không liệt kê, không bảng, không mô tả lại nội dung dashboard bằng chữ.
- Dashboard phải **ĐẦY ĐỦ**: đủ mọi lần khám (`count` lần), không bỏ phần nào giữa chừng. Thà gọn còn hơn cụt.

## Nên có gì trên dashboard (gợi ý, tự do biến tấu — nhưng tất cả PHẢI trong khối html ở trên)
